...
##########################################
EMBEDDING_DIM = 300    
BATCH_SIZE = 256 # 3가지 모델 모두 통일하자.    
NB_EPOCHS = 20
NB_REAL_EX = 3 # 실제 실험 횟수 
MODEL = 'lai_rcnn' 
...
""" DEFINE MODEL """
######################################    
#max_length = len(Xtrain[0]) # rcnn모델에서는 필요없음.
vocab_size = len(tokenizer.word_index) + 1
document = Input(shape = (None, ), dtype = "int32")
left_context = Input(shape = (None, ), dtype = "int32")
right_context = Input(shape = (None, ), dtype = "int32")

embedder = Embedding(vocab_size, # vocab_size
                    EMBEDDING_DIM,
...
# [vocab_size] 뜻은 unknown token으로 대체하겠다는 의도임.
def shift_right(doc):
    return [vocab_size-1] + list(doc[:-1])
def shift_left(doc):
    return list(doc[1:]) + [vocab_size-1]
...
## n-fold cross validation을 하면 아래는 필요없다
...
for train_idx_arr, val_idx_arr in kfold.split(Xtrain, y_temp): # y는 1dim이어야 하므로 여기서 임시로 1dim의 y_temp사용.
    ...
    if USE_VAL_SET == True:    
        ...
        history = model.fit(
                        [Xtrain[train_idx_arr], left_Xtrain[train_idx_arr], right_Xtrain[train_idx_arr]], 
    ...
    with open(FULL_PATH+'results.txt', 'a') as r_f:
        ...
        _, acc2 = model.evaluate([X_test, left_X_test, right_X_test], y_test.values, verbose=0)
